...
contract TestFundAllocation is Test {
    ...
    //Events
    event ProposalCreatedForFunds(
        string indexed _mainDescription,
        uint256 indexed _goal,
        uint256 indexed _deadline
    );
    ...
    function testIfDescriptionOfProposalIsCorrect() external {
        ...
        assertEq(
            keccak256(
                abi.encodePacked(fundAllocation.getMainDescriptionOfProposal(0))
            ),
            keccak256(abi.encodePacked(proposalDescription))
        );
    }
    ...
    function testIfIndivualBalanceOfContributorsGetsUpated() external {
        ...
        fundAllocation.contribute(0, contributeAmount);
        assertEq(
            fundAllocation.getContributionBySpecificContributor(0, alice),
            contributeAmount
        );
    ...
    function testTheNoOfContributorsDontIncreaseWhenContributedTwice()
        external
    {
        vm.prank(bob);
        fundAllocation.createProposal(
    ...
    function testCreateRequestToSpendFundsIsMemberOfDAO()
        external
        BaseForCreateRequestoSpendFunds
    {}

    /**
     * @notice The test to see if CreateRequestToSpendFunds can be called by Proposal Owner only
     */
    function testCreateRequestToSpendFundsCanBeCalledByProposalOwnerOnly()
        external
        BaseForCreateRequestoSpendFunds
    {
        vm.startPrank(bob);
        vm.warp(block.timestamp + fundAllocation.getProposalDeadline(0));
        fundAllocation.CreateRequestToSpendFunds(
            0,
            requestDescription,
            janice,
            requestGoal,
     ...
     */
    function testCreateRequestToSpendFundsCannotWorkWithWrongProposalId()
        external
        BaseForCreateRequestoSpendFunds
    {
        vm.startPrank(bob);
     ...
     */
    function testCreateRequestToSpendFundsCanBeCalledOnlyAfterDeadline()
        external
        BaseForCreateRequestoSpendFunds
    {
     ...
     */
    function testTheValueCannotBeZeroForCreatingRequests()
        external
        BaseForCreateRequestoSpendFunds
    {
        vm.startPrank(bob);
     ...
     */
    function testTheValueCannotBeMoreThanTheRemaningBalance()
        external
        BaseForCreateRequestoSpendFunds
    {
        vm.startPrank(bob);
     ...
     */
    function testTheRecepientAddressCannotBeNull()
        external
        BaseForCreateRequestoSpendFunds
    {
        vm.startPrank(bob);
     ...
     */
    function testTheDescriptionForRequestCannotBeEmpty()
        external
        BaseForCreateRequestoSpendFunds
    {
        vm.startPrank(bob);
     ...
     */
    function testTheRecepientAddressCannotBeTheProposalOwner()
        external
        BaseForCreateRequestoSpendFunds
    {
        vm.prank(bob);
    ...
    function testRequestCreatedDescrptionIsCorrectOrNot() external {
        ...
        assertEq(
            keccak256(
                abi.encodePacked(
                    fundAllocation.getDescriptionOfTheRequest(0, 0)
                )
            ),
    ...
    function testTheDeadlineOfRequestCreated() external {
        ...
        assertEq(
            fundAllocation.getRequestDeadline(0, 0),
     ...
     */
    function testCannotVoteWhenTheRequestIsInStateCompleted()
        external
     ...
     */
    function testTheOwnerOfProposalCannotVote()
        external
        BaseModiferForVoteRequest
    {
        vm.prank(bob);
     ...
     */
    function testTheRecepientOfRequestCannotVote()
        external
        BaseModiferForVoteRequest
    {
        vm.prank(janice);
     ...
     */
    function testIfTheVoteStatevoteInRequestsGetsUpdatedCorectly()
        external
        BaseModiferForVoteRequest
    {
        vm.prank(alice);
        fundAllocation.voteRequest(0, 0, FundAllocation.Vote.Yes);
        assert(
            fundAllocation.getRequestAddressVote(0, 0, msg.sender) ==
                FundAllocation.Vote.Yes
        );
     ...
     */
    function testIfTheVoteStateInRequestsGetsUpdatedCorrectly()
        external
        BaseModiferForVoteRequest
    {
        vm.prank(alice);
        ...
        assertEq(
            fundAllocation.getRequestAddressVoteState(0, 0, daoOwner),
            false
        );
    }
     ...
     */
    function testTheNumberOfYesVotesIncreases()
        external
        BaseModiferForVoteRequest
    {
        vm.prank(alice);
        fundAllocation.voteRequest(0, 0, FundAllocation.Vote.Yes);
        assertEq(
            fundAllocation.getNumberOfYesVotersForSpecificRequest(0, 0),
            1
        );
        assertEq(fundAllocation.getNumberOfNoVotersForSpecificRequest(0, 0), 0);
    }
     ...
     */
    function testNumberOfNoVotesIncrease() external BaseModiferForVoteRequest {
        vm.prank(alice);
        fundAllocation.voteRequest(0, 0, FundAllocation.Vote.No);
     ...
     */
    function testTheTotalNumberOfVotersIncrease()
        external
        BaseModiferForVoteRequest
    {
        vm.prank(alice);
     ...
     */
    function testTheEventEmitsWhenVoted() external BaseModiferForVoteRequest {
        vm.prank(alice);
        vm.expectEmit(true, true, false, false);
     ...
     */
    function testThePreviousVoteGetsClearedAndNewIsAdded()
        external
     ...
     */
    function testTheVoteStateChangesWithChangeInVote()
        external
        BaseConditionForChangeVoteRequest
    {
        vm.prank(alice);
        fundAllocation.changeVoteForRequest(0, 0, FundAllocation.Vote.No);
        assert(
            fundAllocation.getRequestAddressVote(0, 0, alice) ==
                FundAllocation.Vote.No
        );
        ...
        fundAllocation.changeVoteForRequest(0, 0, FundAllocation.Vote.Yes);
        assert(
            fundAllocation.getRequestAddressVote(0, 0, daoOwner) ==
                FundAllocation.Vote.Yes
        );
    }
     ...
     */
    function testVoteCastedEventIsEmittedOrNot()
        external
        BaseConditionForChangeVoteRequest
    {
        vm.prank(alice);
     ...
     */
    function theChangeVoteEmitsAnEvent()
        external
        BaseConditionForChangeVoteRequest
    {
        vm.prank(alice);
     ...
     */
    function testThisFunctionCanOnlyBeCalledByTheOwnerOfProposal()
        external
     ...
     */
    function testWhenNoOfNoVotesIsGreater()
        external
        BaseConditionForMakePayment
    {
     ...
     */
    function testTheActiveRequestsResetsToZeroWhenMoreNoVotes()
        external
        BaseConditionForMakePayment
    {
        address abhishek = makeAddr("abhishek");
     ...
     */
    function testTheNumberOfNoVotesAndYesVotesAreTheSame()
        external
        BaseConditionForMakePayment
    {
        vm.prank(bob);
     ...
     */
    function theWhenTheNoOfYesVotesIsMoreThenCompletedIsTrue()
        external
        BaseConditionForMakePayment
    {
        address abhishek = makeAddr("abhishek");
     ...
     */
    function theTestWhereRaisedAmountAfterMakingPayment()
        external
        BaseConditionForMakePayment
    {
        address abhishek = makeAddr("abhishek");
        ...
        fundAllocation.makePayment(0, 0);
        assertEq(
            fundAllocation.getTotalRaisedAmount(0),
            proposalGoal - requestGoal
        );
    }
     ...
     */
    function theTestToCheckTheAmountSpentTillNow()
        external
        BaseConditionForMakePayment
    {
        address abhishek = makeAddr("abhishek");
        ...
        fundAllocation.makePayment(0, 0);
        assertEq(
            fundAllocation.getTotalAmountSpent(0),
            proposalGoal - requestGoal
        );
    }
     ...
     */
    function theTestWhereTheActiveRequestIsZero()
        external
        BaseConditionForMakePayment
    {
        address abhishek = makeAddr("abhishek");
